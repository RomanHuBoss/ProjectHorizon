...
- `F9` запускает воспроизводимый runtime stress-test `TASK-025`: серия быстрых ревизий принудительно перемещает центр стриминга по детерминированному маршруту и затем возвращает игрока в исходную точку и исходный hysteresis-центр чанка;
...
- Output сообщает планирование, запуск/apply job, revision, worker-время, main-thread-время, отмену устаревших результатов и итог stress-test.
...
Пользователь подтвердил предыдущие исправления геометрии и диагностических режимов. После внедрения фоновой генерации локальная сборка выявила конфликт `Timer`; он устранён явным использованием `Godot.Timer` и точечными alias для threading-типов. Для воспроизводимой приёмки добавлен встроенный stress-test `TASK-025`. После сборки нужно запустить сцену, дождаться исходных `9/9`, нажать `F9` и получить `PASS`; затем вручную проверить collision и визуальные LOD-стыки.
...
6. Дождаться `Активно: 9/9`, `очередь: 0`, `workers: 0/N`, затем нажать `F9` и дождаться строки `TASK-025 stress (F9): PASS`.
7. Проверить диагностические режимы: `F1` — цвет, `F2` — мировая сетка, `F3` — wireframe, `F4` — границы чанков.
8. Для повторной проверки Прототипа A открыть `Scenes/DebugWorld.tscn` и нажать `F6`.
