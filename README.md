...
В текущей итерации реализована основа `noise → mesh → collision`:

- отдельная сцена `TerrainChunkPrototype`;
- `FastNoiseLite` с детерминированным seed, OpenSimplex2 и FBM;
- сетка `33 × 33` вершины;
- `2048` треугольников без соединительных элементов;
- процедурный `ArrayMesh` с UV и нормалями;
- отдельная trimesh-коллизия на `StaticBody3D`;
- существующий игрок для проверки падения, ходьбы и прыжка по рельефу;
- вывод seed, числа вершин, треугольников и времени генерации в Godot Output.
...
LOD, фоновая генерация и выгрузка будут добавляться следующими итерациями после локального smoke test текущей основы.
