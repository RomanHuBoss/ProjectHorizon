...
- входящие чанки создаются первыми, обновления выполняются по одному через idle-таймер, исходящие чанки удаляются последними;
...
- диагностические переключатели перестраивают только visual mesh и overlay, не затрагивая collision;
- HUD показывает координату игрока, состояние перехода, длину очереди, число активных чанков, распределение LOD и текущий диагностический режим;
- Output разделяет планирование и завершение каждой ревизии стриминга и сообщает переключения диагностики.
...
Перед переходом к фоновой генерации требуется локально проверить геометрию в новых диагностических режимах: отсутствие щелей и разрыва сетки на стыках, согласованность нормалей и отсутствие одномоментной перестройки всей активной зоны. После подтверждения следующая техническая задача Прототипа B — вынести расчёт высот и массивов вершин в отменяемые фоновые задания, оставив изменение `SceneTree`, mesh и collision в основном потоке.
