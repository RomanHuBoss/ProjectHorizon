...
Текущий этап — **Прототип A: персонаж**.

Реализованы:

- Godot .NET-проект `Game.Client`;
- тестовая сцена `DebugWorld` с физическим полом и направленным светом;
- игрок на `CharacterBody3D` с капсульной коллизией;
- единственная камера от первого лица в иерархии игрока;
- Input Map для WASD и прыжка;
- движение, гравитация, прыжок и обзор мышью;
- захват и освобождение курсора;
- `Nullable` для C#;
- Git и Git LFS;
- тестовая геометрия для проверки столкновений;
- базовое взаимодействие по клавише `E` через `RayCast3D`;
- тестовый интерактивный терминал с визуальным переключением состояния;
- простая hitscan-стрельба по ЛКМ;
- отдельный компонент `HitscanWeapon` с ограничением частоты выстрелов;
- тестовая цель с красной вспышкой и журналированием попаданий.

До приёмки Прототипа A остаются повторная локальная сборка после исправления `CS0019`, проверка стрельбы, фиксация результатов сборки и полный приёмочный прогон.
