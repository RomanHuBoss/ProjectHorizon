...
- высоты рассчитываются в единых мировых координатах, поэтому границы соседних чанков согласованы;
- визуальные skirts закрывают возможные щели между разными LOD;
- коллизия создаётся отдельно и сохраняет разрешение `33 × 33` в активной физической зоне;
- при переходе игрока через границу чанка создаётся новый ближний ряд, а дальний ряд освобождается;
- HUD показывает координату игрока, число активных чанков, распределение LOD и seed;
- Output сообщает генерацию, смену LOD, создание и выгрузку чанков.
...
Следующая техническая задача Прототипа B — вынести расчёт высот и массивов вершин в отменяемые фоновые задания, оставив изменение `SceneTree`, mesh и collision в основном потоке.
