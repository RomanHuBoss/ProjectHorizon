...
Первая ступень Прототипа D реализует свободный космический полёт на упрощённой
аркадной физике из раздела 14.1 PDF-ТЗ:

- тяга вперёд/назад;
- боковые и вертикальные импульсные двигатели;
- рыскание, тангаж и крен;
- форсаж;
- торможение;
- автоматическая угловая стабилизация;
- chase и cockpit cameras;
- автономный измеримый smoke-test.
...
J         автоматический TASK-043 flight test
```

Текущая итерация не включает атмосферную физику, посадку, взлёт и переход к
поверхности. Эти функции реализуются следующими изолированными задачами.
...
4. Проверить ручной free-flight:
   ...
   - F2 — chase/cockpit camera;
   - R — reset.
5. Нажать `J` и дождаться `TASK-043 flight (J): PASS`.
6. Клавиша `H` переключает compact, detailed и hidden HUD корабля.
7. Для регрессии Прототипа C открыть
...
- свободный аркадный полёт — `IMPLEMENTED`;
- тяга, импульсные двигатели, тангаж/рыскание/крен — `IMPLEMENTED`;
- форсаж, торможение и стабилизация — `IMPLEMENTED`;
- chase/cockpit cameras — `IMPLEMENTED`;
- посадка — `NOT_STARTED`;
- взлёт — `NOT_STARTED`;
- переход к поверхности и атмосферный режим — `NOT_STARTED`.
