...
`0.1.0-alpha.220` replaces the owner-rejected TASK-216/TASK-218 visual result instead of polishing it. The crystal/ice authoring axis bug is corrected so longitudinal resource geometry is actually Y-up; Crystal LOD0 now measures about 1.43× taller than its maximum footprint and Ice about 1.36×. Explorer/Interceptor primary hull and wing surfaces are remapped to a light industrial-alloy atlas slot, with dark values reserved for canopy/recesses. Runtime resource material application now distinguishes host matrix, crystal/blade, core/vein/throat and salvage-metal roles instead of flattening every imported submesh to the same shading. F5 adds `TASK-220 production art recovery acceptance`; manual visual review is still mandatory. See `docs/PRODUCTION_ART_RECOVERY.md`.

## TASK-218 — Production PBR Texture Atlas & Resource Visual Diversity

`0.1.0-alpha.218` adds the next production-art layer without pretending that TASK-216 manual visual acceptance is complete. Explorer, Interceptor and the orbital station now carry deterministic UV0 and a single reusable four-map 1024 PBR atlas material per GLB (BaseColor / Normal / MetallicRoughness / Emission). The resource library expands from five to ten silhouette families — Ore, Salvage, Crystal, Fiber, Organic, Ice, Gas, Salt, Glass and Exotic — with three LODs each and semantic routing for all 42 resource definitions. Collision remains separate. The owner later rejected this visual result; TASK-218 is retained only as a regression contract and is superseded by TASK-220. See `docs/PRODUCTION_SURFACE_ART.md` and `docs/PRODUCTION_ART_RECOVERY.md`.

## TASK-216 — Production 3D Model Art Overhaul

`0.1.0-alpha.216` raises the visual-density baseline of the Explorer, Interceptor and orbital station and moves surface/cave/starter resources to five production GLB families with three LOD levels each. Selected close-range greeble modules use reviewed Kenney Space Kit CC0 authoring inputs; shipping collision/mount markers remain unchanged and raw authoring OBJ files are never runtime-loaded. Resource primitive meshes are emergency fallback only. The owner later rejected this visual result; TASK-216 is superseded by TASK-220 while its GLB/LOD/collision checks remain regression gates. See `docs/PRODUCTION_MODEL_ART_OVERHAUL.md` and `docs/PRODUCTION_ART_RECOVERY.md`.
