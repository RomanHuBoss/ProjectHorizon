...
Основа cube sphere принята по локальной runtime-проверке: `6/6` граней, `6/6` collision, `388/388` seam-пар, максимальные ошибки позиций и нормалей равны нулю. Реализована следующая итерация — радиальная система персонажа:

- отдельный `PlanetaryPlayerController` на `CharacterBody3D`;
- гравитация всегда направлена к центру планеты, а не по глобальной оси `-Y`;
- `UpDirection` и локальная ось `Y` персонажа согласуются с радиальным направлением от центра;
- ориентация плавно перестраивается при изменении положения на сфере;
- WASD проецируется в касательную плоскость поверхности;
- третьелицевая камера вращается в локальной системе персонажа;
- `Space` выполняет радиальный прыжок наружу от центра;
- `R` возвращает персонажа в исходную тестовую точку;
- `F2` переключает камеру игрока и прежний орбитальный обзор;
- в обзорном режиме `Space` приостанавливает/возобновляет орбиту;
- HUD показывает радиальное расстояние, контакт с поверхностью, касательную скорость и ошибку выравнивания локального Up.
...
Новая кодовая часть ожидает локальной runtime-приёмки. После подтверждения радиальной гравитации следующим шагом будет устойчивое прохождение границ граней, затем floating origin и quadtree LOD.
...
6. Убедиться, что HUD показывает `Грани: 6/6`, `collision: 6/6`, `швы: PASS` и `Радиальная система: PASS` после выравнивания персонажа.
7. Персонаж стартует сбоку от планеты, должен притянуться к поверхности и остановиться на collision без зависимости от глобальной оси `Y`.
8. Проверить WASD и мышь: движение должно оставаться касательным к сфере, а локальный Up — направленным от центра.
9. Нажать `Space`: персонаж должен прыгнуть радиально наружу и вернуться на поверхность. `R` возвращает его в исходную тестовую точку.
10. Нажать `F2`: должна включиться обзорная орбитальная камера. В этом режиме `Space` ставит орбиту на паузу; повторный `F2` возвращает управление игроком.
11. Нажать `F1`: цвета граней должны смениться визуализацией радиальных нормалей без изменения geometry/collision.
