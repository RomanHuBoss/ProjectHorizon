...
- HUD показывает координату игрока, состояние перехода, длину очереди, число активных чанков и распределение LOD;
- Output разделяет планирование и завершение каждой ревизии стриминга.
...
Перед переходом к фоновой генерации требуется локально подтвердить исправление двух обнаруженных runtime-дефектов: отсутствие видимых щелей LOD и отсутствие одномоментной перестройки всей сетки. После подтверждения следующая техническая задача Прототипа B — вынести расчёт высот и массивов вершин в отменяемые фоновые задания, оставив изменение `SceneTree`, mesh и collision в основном потоке.
...
6. Для повторной проверки Прототипа A открыть `Scenes/DebugWorld.tscn` и нажать `F6`.
