...
| B. Чанк рельефа | `IN_PROGRESS` | Фоновая генерация и встроенный stress-test `F9` реализованы; конфликт `Godot.Timer`/`System.Threading.Timer` устранён; требуется локальная сборка и получение runtime-результата `PASS` |
| C. Сферическая планета | `NOT_STARTED` | Не начинался |
| D. Корабль | `NOT_STARTED` | Не начинался |
| E. Сохранение | `NOT_STARTED` | Не начинался |

**Вывод:** фоновая генерация данных чанков реализована, блокирующий дефект `CS0104` устранён, а для `TASK-025` добавлен детерминированный runtime stress-test с итогом `PASS/FAIL`. Присваивать `VERIFIED` пока нельзя: требуется локальная сборка и фактический прогон `F9`. Прототип A сохранён в `Scenes/DebugWorld.tscn`.

---

## 3. Результат текущей итерации от 2026-08-01

### 2026-08-01 — встроенный stress-test асинхронного стриминга

**Основание:** `TASK-025`, требования `PB-050`–`PB-052`, ограничения разделов 10.3–10.4 и состав Прототипа B PDF-ТЗ.

**Реализовано:**

- клавиша `F9` запускает воспроизводимый тест серии быстрых streaming-revision;
- перед тестом менеджер ожидает полного завершения текущего плана и временно отключает физический процесс игрока;
- центр стриминга проходит детерминированный маршрут из `StressTestRevisionCount` позиций и в финале возвращается к исходному чанку;
- интервалы ревизий и timeout настраиваются через `StressTestStepIntervalSeconds` и `StressTestTimeoutSeconds`;
- добавлены накопительные счётчики worker-ошибок, отмен и stale-результатов, не обнуляемые при каждой новой revision;
- ошибки worker учитываются даже тогда, когда результат успел стать stale либо находился в ready-очереди при переключении revision;
- итоговая проверка требует пустых pending/apply/ready/completed-очередей, `workers=0`, совпадения completed revision с текущей и отсутствия worker-ошибок;
- каждый активный чанк сопоставляется с последним `ChunkSpec`; лишний, отсутствующий либо оставшийся от stale-revision чанк приводит к `FAIL`;
- тест требует фактически наблюдаемой отмены либо stale-результата, иначе сообщает, что нагрузка была недостаточной;
- итог `PASS/FAIL` выводится в HUD и Output с числами revision, cancelled, stale, failed и active chunks;
- игрок возвращается в исходную позицию и исходный hysteresis-центр чанка, а его физический процесс восстанавливается;
- HUD учитывает все очереди и готовые результаты при определении состояния перехода.

**Статусы:**

- кодовая часть `TASK-025` реализована;
- `TASK-025` остаётся `IN_PROGRESS` до фактического результата `F9: PASS`;
- `TASK-005` остаётся незавершённой до локальной сборки этой редакции;
- требования не переводились в `VERIFIED` без runtime-доказательства.

**Ограничение проверки:** в рабочей среде отсутствуют Godot и .NET SDK. Выполнены статическая проверка структуры C#, баланса скобок, маршрута ревизий, критериев PASS/FAIL и целостности проекта.
...
| `PB-050` | Высоты/вершины считаются в фоне | `IMPLEMENTED` | `TerrainChunkDataBuilder` вычисляет noise-высоты, глобальные нормали, UV и индексы в `Task.Run`; `F9` создаёт серию фоновых ревизий для воспроизводимого runtime-контроля |
| `PB-051` | SceneTree изменяется только в main thread | `IMPLEMENTED` | Worker возвращает только `TerrainMeshData`; `AddChild`, `ArrayMesh`, material и collision создаются в `ProcessOperationQueue`; stress-test валидирует итоговый активный набор после серии worker-revision |
| `PB-052` | Отмена устаревшей фоновой генерации | `IMPLEMENTED` | На каждый план создаётся `CancellationTokenSource`; используются `jobId` и revision; `F9` требует наблюдаемой отмены/stale и проверяет, что финальные чанки соответствуют только последней revision |
...
| `PB-ACC-007` | Активны 9 чанков | `IN_PROGRESS` | `F9` автоматически проверяет соответствие полного активного набора `_desiredSpecs`; требуется получить локальный `PASS` при `ActiveRadius=1` |
| `PB-ACC-008` | Стриминг возвращается к размеру активной сетки | `IN_PROGRESS` | `F9` проверяет пустые очереди, `workers=0`, completed=current revision и отсутствие лишних/пропавших чанков; нужен фактический `PASS` |
...
|---:|---|---|---|
| 1 | `TASK-025` | Собрать проект и запустить встроенный stress-test `F9` | HUD показывает `PASS`; активный набор полный, workers и все очереди — `0`; stale job не применяется; worker-ошибок нет |
...
**Реализовано в этой итерации:** встроенный воспроизводимый stress-test асинхронного менеджера чанков с итогом `PASS/FAIL`.  
**Текущая задача:** локальная сборка и получение `TASK-025 stress (F9): PASS` по разделу 10.
...
4. После инициализации дождаться `стабильно`, `очередь: 0`, `workers: 0/N`, `ошибки: 0` и `Активно: 9/9`.
5. Нажать `F9`. Игрок автоматически пройдёт серию быстрых смен центра; ручное управление на время теста блокируется.
6. В Output должны появиться `Terrain async stress test: started`, несколько новых streaming revision и сообщения `discarded stale` либо отменённые job.
7. Дождаться строки HUD `TASK-025 stress (F9): PASS`. Итоговая строка Output должна содержать `failed=0`, полный active set, `pending=0`, `apply=0`, `ready=0`, `completed=0`, `workers=0`.
8. Любой `FAIL`, timeout, `Terrain worker failed`, исключение `SceneTree`, mesh или collision считать незакрытым дефектом; сохранить полный Output.
9. Убедиться, что после теста игрок вернулся в исходную точку, управление восстановилось, HUD снова показывает `Активно: 9/9`.
10. Нажать `F1`–`F4`. Переключение диагностики должно происходить без запуска новых worker-job и без изменения collision.
11. Медленно пройти границу чанка: входящие чанки появляются после готовности worker-результатов, исходящий ряд не удаляется раньше завершения create/update.
12. Включить `Debug → Visible Collision Shapes`: collision совпадает с поверхностью, не содержит overlay и skirts и не исчезает при visual-only LOD update.
13. Проверить режимы высоты, LOD и нормалей на четырёх границах центрального чанка: просветов, световых швов и разрыва сетки нет.
14. Перезапустить сцену: при том же seed геометрия и цветовое распределение совпадают.
15. Записать результат сборки, итог `F9`, значения `worker/main`, число workers и SHA коммита в раздел 11.
