...
| C. Сферическая планета | `IN_PROGRESS` | Основа cube sphere принята runtime; реализованы радиальная гравитация, локальный Up, касательное управление и планетарная камера, ожидающие приёмки |
| D. Корабль | `NOT_STARTED` | Не начинался |
| E. Сохранение | `NOT_STARTED` | Не начинался |

**Вывод:** пользователь подтвердил основу cube sphere: HUD показал `6/6` граней, `6/6` collision, `388/388` seam-пар, `Δpos max=0`, `Δnormal max=0`, видимых разрывов нет, диагностические режимы работают. Поэтому `TASK-027`, `TASK-028`, базовые требования `PC-001`–`PC-030` и их приёмка переведены в `VERIFIED`. Текущая итерация реализует `TASK-029`: радиальную гравитацию, локальную ориентацию игрока и касательное управление; код ожидает локального smoke-test.
...
- новая радиальная система ожидает локального smoke-test;
- Прототип C остаётся `IN_PROGRESS` до устойчивой ходьбы через границы граней, floating origin и LOD.
...
| `PC-040` | Отдельный планетарный `CharacterBody3D` | `IMPLEMENTED` | `PlanetaryPlayer.tscn`, `PlanetaryPlayerController.cs`; старт сбоку от планеты |
| `PC-041` | Гравитация направлена к центру планеты | `IMPLEMENTED` | `GravityDirection = -RadialUp`; ускорение применяется в `_PhysicsProcess` |
| `PC-042` | Локальный Up направлен от центра | `IMPLEMENTED` | `UpDirection = RadialUp`; basis плавно slerp-выравнивается с радиальной нормалью |
| `PC-043` | Камера и движение не зависят от глобальной оси Y | `IMPLEMENTED` | движение проецируется в касательную плоскость; yaw выполняется вокруг локальной Y; камера является дочерней локальной иерархии |
| `PC-044` | Диагностика радиальной системы | `IMPLEMENTED` | HUD: `r`, ground, касательная скорость, `Δup`, `PASS/ALIGNING`; `F2` и `R` |
| `PC-ACC-001` | Проект собирается без ошибок | `VERIFIED` | Подтверждено запуском основы cube sphere; для новой радиальной части требуется повторная сборка |
...
| `PC-ACC-010` | Радиальная версия собирается без ошибок | `IN_PROGRESS` | Выполнить локальную сборку текущего снимка |
| `PC-ACC-011` | Игрок притягивается к боковой поверхности | `IN_PROGRESS` | После старта игрок должен упасть по направлению к центру и остановиться на collision |
| `PC-ACC-012` | Ошибка ориентации Up не превышает 1° | `IN_PROGRESS` | HUD должен показать `Радиальная система: PASS` и `Δup ≤ 1°` |
| `PC-ACC-013` | WASD и камера работают в касательной системе | `IN_PROGRESS` | Проверить движение и мышь на боковой части сферы |
| `PC-ACC-014` | Прыжок, reset и переключение камер работают | `IN_PROGRESS` | Проверить `Space`, `R`, `F2` и обзорную паузу |
| `PC-ACC-015` | Нет критических ошибок Output | `IN_PROGRESS` | Проверить исключения, physics warnings и отсутствующие узлы |
...
**Зафиксировано как `VERIFIED` по прямому подтверждению пользователя:** `TASK-005`, `TASK-009`, `TASK-011`, `TASK-023`–`TASK-028`; Прототипы A и B; геометрическая основа Прототипа C.

| Приоритет | ID | Задача | Результат |
|---:|---|---|---|
| 1 | `TASK-029` | Выполнить runtime smoke-test радиальной гравитации и ориентации | Игрок притягивается к боковой поверхности; HUD `Радиальная система: PASS`, `Δup ≤ 1°`; WASD, мышь, `Space`, `R`, `F2` работают |
| 2 | `TASK-030` | Довести ходьбу по collision cube sphere через границы граней | Игрок устойчиво проходит межгранные швы без провала, рывка ориентации или потери floor contact |
| 3 | `TASK-032` | Реализовать прототип floating origin | При удалении более чем на 2048 м локальная сцена переносится, логические координаты сохраняются |
| 4 | `TASK-033` | Начать quadtree LOD граней и согласование соседей | Независимые patches, LOD-переходы и диагностируемые швы без отверстий |
| 5 | `TASK-006` | Записать SHA контрольного коммита | Журнал содержит Git-доказательство принятой редакции |

**Реализовано в этой итерации:** закрытие `TASK-027`/`TASK-028` и кодовая часть `TASK-029`.  
**Текущая задача:** локально выполнить smoke-test радиальной системы по разделу 10.
...
2. Нажать `F5`; должна открыться сцена `CubeSpherePrototype` в режиме камеры игрока.
3. Убедиться, что Output содержит `CubeSphere foundation` и `Planetary player initialized` без исключений.
4. Игрок стартует сбоку от сферы. Не нажимая клавиши, дождаться притяжения к поверхности и устойчивого контакта с collision.
5. HUD должен показать `Грани: 6/6`, `collision: 6/6`, `швы: PASS`, затем `Радиальная система: PASS`; значение `Δup` должно быть не более `1°`.
6. Нажать WASD: движение должно идти вдоль касательной поверхности, а не в плоскости глобальных XZ. Персонаж не должен самопроизвольно падать в направлении глобальной `-Y`.
7. Повернуть мышь: yaw должен выполняться вокруг локального радиального Up, pitch — вокруг локальной оси камеры.
8. Нажать `Space`: игрок должен получить импульс наружу от центра и вернуться на поверхность.
9. Нажать `R`: игрок должен вернуться в исходную боковую точку и повторно выровняться.
10. Нажать `F2`: включается орбитальный обзор, управление игроком отключается. В обзоре `Space` ставит орбиту на паузу. Повторный `F2` возвращает камеру игрока.
11. Нажать `F1`: диагностика граней/нормалей должна переключиться без изменения collision.
12. Проверить Output на отсутствие C# exceptions, ошибок `SpringArm3D`, `CharacterBody3D`, collision и сообщений о несуществующих узлах.
13. При успешном результате передать screenshot HUD; это позволит перевести `TASK-029`, `PC-040`–`PC-044` и `PC-ACC-010`–`PC-ACC-015` в `VERIFIED`.
14. Для регрессии Прототипов A/B при необходимости запустить их сохранённые сцены через `F6`.
