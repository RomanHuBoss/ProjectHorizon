...
| C. Сферическая планета | `IN_PROGRESS` | Основа cube sphere и радиальная система игрока приняты; реализована устойчивая ходьба через межгранные collision-швы и автоматический seam-test, ожидающий runtime-приёмки |
| D. Корабль | `NOT_STARTED` | Не начинался |
| E. Сохранение | `NOT_STARTED` | Не начинался |

**Вывод:** пользователь принял геометрическую основу cube sphere и последующую радиальную систему игрока, после чего запросил следующую итерацию. Поэтому `TASK-027`–`TASK-029`, `PC-001`–`PC-044` и соответствующие критерии приёмки переведены в `VERIFIED`. Текущая итерация реализует `TASK-030`: устойчивый контакт `CharacterBody3D` при переходах между шестью независимыми collision-гранями и автоматический seam traversal test.

---

## 3. Результат текущей итерации от 2026-08-01

### 2026-08-01 — приёмка радиальной системы и ходьба через швы `TASK-030`

**Основание:** раздел 13.4 и Прототип C раздела 39 PDF-ТЗ; прямое подтверждение пользователя после получения редакции с радиальной гравитацией.

**Изменения статусов по подтверждению пользователя:**

- `TASK-029`: `IMPLEMENTED` → `VERIFIED`;
- `PC-040`–`PC-044` → `VERIFIED`;
- `PC-ACC-010`–`PC-ACC-015` → `VERIFIED`;
- радиальная гравитация, локальный Up, касательное управление, прыжок, reset и переключение камер считаются принятыми;
- `TASK-030` начата как следующий функциональный шаг Прототипа C.

**Реализовано в `TASK-030`:**

- добавлены пять локальных `RayCast3D`-проб под капсулой игрока: центральная и четыре смещённые по касательным осям;
- контакт с поверхностью больше не зависит только от одного результата `IsOnFloor()` при смене collision-граня;
- увеличены `FloorSnapLength`, `SafeMargin` и `MaxSlides`, включена постоянная скорость на полу;
- добавлены кратковременный ground grace и направленная к центру скорость прилипания, не мешающая радиальному прыжку;
- при краткой потере floor classification применяется `ApplyFloorSnap()`;
- текущая грань определяется по доминирующей компоненте радиального направления с гистерезисом, исключающим дребезг на границе;
- HUD показывает текущую грань, отдельные состояния `floor`/`probe` и число межгранных переходов;
- клавиша `T` запускает автоматический маршрут по большому кругу, рассчитанный на четыре подтверждённых смены граней;
- seam-test контролирует максимальную длительность потери физического контакта и максимальную ошибку локального Up;
- результат выводится как `TASK-030 seam (T): PASS/FAIL`; повторное `T`, `F2` или `R` безопасно останавливает тест;
- после автоматического прогона игрок возвращается в исходную позицию и ориентацию;
- сохранены шесть независимых visual/collision-граней и прежняя геометрическая seam-диагностика `388/388`.

**Статус текущей итерации:**

- `TASK-030` — `IMPLEMENTED`;
- `PC-050`–`PC-053` — `IMPLEMENTED`;
- `TASK-031` назначена runtime-приёмкой: локальная сборка, ручной переход границы и автоматический итог `TASK-030 seam (T): PASS`;
- Прототип C остаётся `IN_PROGRESS` до приёмки ходьбы, floating origin и quadtree LOD.

### 2026-08-01 — приёмка cube sphere и радиальная гравитация `TASK-029`

**Основание:** разделы 9.1–9.3, 13.4 и Прототип C раздела 39 PDF-ТЗ; предоставленный пользователем screenshot и подтверждение корректной работы основы cube sphere.

**Полученное runtime-доказательство `TASK-028`:**

- сцена запустилась без ошибок компиляции;
- HUD: `Грани: 6/6`, `collision: 6/6`;
- сетка: `33×33`, вершины: `6534`, треугольники: `12288`;
- `Швы: PASS`, пары: `388/388`;
- `Δpos max: 0,00E+000`, `Δnormal max: 0,00E+000`;
- пользователь подтвердил корректный визуальный результат и работу сцены.

**Изменения статусов по runtime-подтверждению:**

- `TASK-027`: `IMPLEMENTED` → `VERIFIED`;
- `TASK-028`: `IN_PROGRESS` → `VERIFIED`;
- `PC-001`, `PC-010`–`PC-014`, `PC-020`, `PC-021`, `PC-030` → `VERIFIED`;
- `PC-ACC-001`–`PC-ACC-006` → `VERIFIED`.

**Реализовано в `TASK-029`:**

- добавлена отдельная сцена `Scenes/Planet/PlanetaryPlayer.tscn` и контроллер `PlanetaryPlayerController`;
- гравитация вычисляется как нормализованный вектор от игрока к центру планеты;
- `CharacterBody3D.UpDirection` каждый physics tick задаётся радиальным направлением наружу;
- локальная ось `Y` персонажа плавно согласуется с радиальным Up без привязки к глобальной оси мира;
- сохраняется касательное направление взгляда, а yaw выполняется вокруг локальной радиальной оси;
- WASD проецируется в касательную плоскость сферы;
- прыжок задаёт скорость вдоль радиального Up;
- добавлена третьелицевая камера со `SpringArm3D`, вращающаяся в локальной системе персонажа;
- `F2` переключает камеру игрока и диагностический орбитальный обзор;
- в режиме игрока `Space` отвечает за прыжок, в обзорном режиме — за паузу орбиты;
- `R` возвращает игрока в исходную боковую точку над поверхностью;
- HUD показывает радиальное расстояние, контакт с поверхностью, касательную скорость, ошибку Up и итог `PASS/ALIGNING`;
- прежняя cube-sphere seam-диагностика, `F1` и collision сохранены.

**Статус текущей итерации:**

- `TASK-029` позднее принята пользователем и переведена в `VERIFIED`;
- Прототип C остаётся `IN_PROGRESS` до приёмки ходьбы через границы граней, floating origin и LOD.

### 2026-08-01 — приёмка `TASK-026` и основа cube sphere `TASK-027`

**Основание:** разделы 9.1–9.4 и Прототип C раздела 39 PDF-ТЗ; очередь журнала после завершения длительного soak-test Прототипа B.

**Полученное runtime-доказательство `TASK-026`:**

- HUD показал `TASK-026 soak (P): PASS`;
- длительность — `121 с`;
- выполнено `82` перехода;
- `managedDelta=0,0 MB`;
- финальные ресурсы: `mesh=9`, `collision=9`;
- после теста: `Активно: 9/9`, `очередь: 0`, `workers: 0/4`, `ошибки: 0`, переход `стабильно`;
- пользователь подтвердил завершение тестов и корректную работу.

**Изменения статусов по runtime-подтверждению:**

- `TASK-026`: `IN_PROGRESS` → `VERIFIED`;
- `PB-061`: `IMPLEMENTED` → `VERIFIED`;
- Прототип B: `IN_PROGRESS` → `VERIFIED`;
- `TASK-027` начата как переход к Прототипу C.

**Реализовано в `TASK-027`:**

- добавлена отдельная стартовая сцена `Scenes/Planet/CubeSpherePrototype.tscn`;
- создан чистый построитель данных `CubeSphereMeshBuilder`, не зависящий от `Node` и не выполняющий работу в `_Process`;
- планета строится из шести независимых граней `+X/-X/+Y/-Y/+Z/-Z`;
- каждая грань использует сетку `33 × 33` и `2048` треугольников, суммарно `6534` вершины и `12288` треугольников;
- вершины куба нормализуются и проецируются на сферу;
- детерминированная высота добавляется вдоль радиального направления через 3D `FastNoiseLite` OpenSimplex2 + FBM;
- общая функция высоты использует нормализованное направление, поэтому дублированные вершины соседних граней получают одинаковые координаты;
- для каждой грани создаётся отдельный `MeshInstance3D`, формирующий основу будущего quadtree;
- для каждой грани создаётся отдельная `ConcavePolygonShape3D` внутри общего `StaticBody3D`;
- встроенная seam-диагностика сравнивает дублированные граничные позиции и радиальные нормали и выводит максимальную ошибку;
- HUD показывает число граней/collision, радиус, амплитуду, seed, размер сетки, вершины, треугольники и `Швы: PASS/FAIL`;
- `F1` переключает цвета граней и радиальные нормали без изменения collision;
- `Space` останавливает/возобновляет орбитальное движение камеры, сама планета и StaticBody остаются неподвижными;
- `project.godot` переключён на Прототип C; сцены Прототипов A и B сохранены для регрессии;
- README и журнал актуализированы.

**Статус текущей итерации:**

- кодовая часть `TASK-027` — `IMPLEMENTED`;
- Прототип C — `IN_PROGRESS`;
- `TASK-028` назначена runtime-проверкой: сборка, `6/6` граней, `6/6` collision, `Швы: PASS`, отсутствие видимых разрывов и корректная работа `F1`/`Space`.

**Ограничение проверки:** в рабочей среде отсутствуют Godot и .NET SDK. Выполнены статический аудит C#, математическая проверка ориентации треугольников всех шести граней, проверка ресурсов сцены и целостности проекта. Фактический запуск выполняется локально пользователем.
...
| `TOOL-002` | 1.3 | Производственный код на C# | `IMPLEMENTED` | `DebugWorld.cs`, `PlayerController.cs`, terrain-компоненты, `CubeSpherePrototype.cs`, `CubeSphereMeshBuilder.cs`, `PlanetaryPlayerController.cs`, компоненты взаимодействия и боя | Проверять в CI |
...
| `PC-050` | Многоточечная поддержка контакта на collision-швах | `IMPLEMENTED` | Пять `RayCast3D`-проб под капсулой; центральная и четыре смещённые точки |
| `PC-051` | Удержание поверхности при смене collision-граня | `IMPLEMENTED` | `FloorSnapLength`, `SafeMargin`, `ApplyFloorSnap`, ground grace и радиальная adhesion speed |
| `PC-052` | Диагностика текущей грани и переходов | `IMPLEMENTED` | HUD выводит `+X/-X/+Y/-Y/+Z/-Z`, `floor`, `probe` и счётчик переходов |
| `PC-053` | Автоматический тест ходьбы через швы | `IMPLEMENTED` | `T`: маршрут по большому кругу, 4 перехода, контроль contact gap и `Δup`, итог `PASS/FAIL` |
...
| `PC-ACC-020` | Игрок вручную пересекает границу collision-граней | `IN_PROGRESS` | Проверить переход при включённых цветах граней без провала и заметного рывка |
| `PC-ACC-021` | Многоточечный probe сохраняет контакт | `IN_PROGRESS` | На шве допустима смена `floor`, но `probe`/ground не должны пропадать дольше 0,12 с |
| `PC-ACC-022` | Автоматический seam-test проходит минимум 4 границы | `IN_PROGRESS` | HUD должен показать `TASK-030 seam (T): PASS crossings=4` |
| `PC-ACC-023` | В тесте нет рывка ориентации | `IN_PROGRESS` | Итоговый `Δup` не более 3°; визуально камера не переворачивается |
| `PC-ACC-024` | После теста состояние восстановлено без ошибок | `IN_PROGRESS` | Игрок возвращается в исходную точку; Output не содержит C# exceptions |

Устойчивое прохождение границ граней, floating origin и quadtree LOD будут добавляться следующими изолированными итерациями.
...
**Зафиксировано как `VERIFIED` по прямому подтверждению пользователя:** `TASK-005`, `TASK-009`, `TASK-011`, `TASK-023`–`TASK-029`; Прототипы A и B; геометрическая основа и радиальная система игрока Прототипа C.

| Приоритет | ID | Задача | Результат |
|---:|---|---|---|
| 1 | `TASK-031` | Выполнить runtime-приёмку ходьбы через межгранные швы | Ручной переход без провала; `TASK-030 seam (T): PASS`, минимум 4 перехода, contact gap ≤ 0,12 с, `Δup ≤ 3°` |
| 2 | `TASK-032` | Реализовать прототип floating origin | При удалении более чем на 2048 м локальная сцена переносится, логические координаты сохраняются |
| 3 | `TASK-033` | Начать quadtree LOD граней и согласование соседей | Независимые patches, LOD-переходы и диагностируемые швы без отверстий |
| 4 | `TASK-006` | Записать SHA контрольного коммита | Журнал содержит Git-доказательство принятой редакции |

**Реализовано в этой итерации:** приёмка `TASK-029` и кодовая часть `TASK-030`.
**Текущая задача:** выполнить локальный seam smoke-test по разделу 10.
...
2. Нажать `F5`; сцена должна запуститься в режиме камеры игрока без исключений.
3. Дождаться контакта с поверхностью: HUD должен показать `Радиальная система: PASS`, `ground=да` и обычно `probe=да`.
4. В режиме цветов граней пройти WASD через визуальную границу двух граней. Допустима краткая смена `floor`, но игрок не должен проваливаться, подпрыгивать или терять управление.
5. Убедиться, что строка `Грань:` меняется, а счётчик `переходы` увеличивается ровно после фактического пересечения.
6. Нажать `T`. Игрок сначала стабилизируется, затем автоматически движется по замкнутой сферической траектории.
7. Во время теста не нажимать WASD/мышь. HUD должен последовательно показывать `RUNNING 0/4` … `RUNNING 4/4`.
8. Ожидаемый итог: `TASK-030 seam (T): PASS crossings=4`, `gap ≤ 0,12 с`, `Δup ≤ 3°`.
9. После завершения игрок должен вернуться в исходную позицию и продолжить обычное ручное управление.
10. Повторное `T` во время теста должно безопасно остановить его; `F2` и `R` также не должны оставлять автопилот активным.
11. Проверить `Space`, `R`, `F1`, `F2` после теста — предыдущая функциональность должна сохраниться.
12. Проверить Output: допустимы диагностические сообщения `Planet face transition`; C# exceptions, physics errors и отсутствующие узлы недопустимы.
13. Передать screenshot итогового HUD. После `PASS` можно перевести `TASK-030`, `TASK-031`, `PC-050`–`PC-053` и `PC-ACC-020`–`PC-ACC-024` в `VERIFIED`.
14. Для регрессии Прототипов A/B при необходимости запустить сохранённые сцены через `F6`.
15. После коммита записать SHA в `TASK-006`.
