...
| C. Сферическая планета | `IN_PROGRESS` | Реализована первая основа cube sphere: 6 граней, `33 × 33`, радиальная проекция, детерминированная высота, collision и seam-диагностика |
| D. Корабль | `NOT_STARTED` | Не начинался |
| E. Сохранение | `NOT_STARTED` | Не начинался |

**Вывод:** пользователь подтвердил `TASK-026 soak (P): PASS` со значениями `121 с`, `moves=82`, `managedDelta=0,0 MB`, `mesh=9`, `collision=9`; после теста получены `9/9`, `queue=0`, `workers=0/4`, `ошибки=0`. На этом основании `TASK-026`, `PB-061` и Прототип B переведены в `VERIFIED`. Текущая итерация начинает Прототип C отдельной сценой cube sphere; её runtime-приёмка назначена `TASK-028`.
...
| `TOOL-002` | 1.3 | Производственный код на C# | `IMPLEMENTED` | `DebugWorld.cs`, `PlayerController.cs`, terrain-компоненты, `CubeSpherePrototype.cs`, `CubeSphereMeshBuilder.cs`, компоненты взаимодействия и боя | Проверять в CI |
...
| `ARCH-006` | 4.3 | Клиент содержит сцены, камеры, управление и адаптеры взаимодействия | `IMPLEMENTED` | `DebugWorld`, `TerrainChunkPrototype`, `CubeSpherePrototype`, `Player`, управление, взаимодействие и бой | Довести Прототип A до приёмки |
...
### 8.1. Прототип C — основа cube sphere

| ID | Требование | Статус | Доказательство / следующее действие |
|---|---|---|---|
| `PC-001` | Отдельная тестовая сцена сферической планеты | `IMPLEMENTED` | `Scenes/Planet/CubeSpherePrototype.tscn`; назначена стартовой |
| `PC-010` | Шесть независимых граней cube sphere | `IMPLEMENTED` | `CubeSphereMeshBuilder` создаёт `+X/-X/+Y/-Y/+Z/-Z`; каждая грань — отдельный `MeshInstance3D` |
| `PC-011` | Сетка `33 × 33` и 2048 треугольников на грань | `IMPLEMENTED` | Экспортируемый `FaceResolution=33`; суммарно `6534` вершины и `12288` треугольников |
| `PC-012` | Проекция куба на сферу | `IMPLEMENTED` | `cubePoint.Normalized()` формирует радиальное направление; позиция равна направлению, умноженному на радиус с высотой |
| `PC-013` | Детерминированный рельеф по радиальному направлению | `IMPLEMENTED` | 3D `FastNoiseLite`, seed `20260801`, OpenSimplex2 + FBM; одинаковое направление даёт одинаковую высоту на границе |
| `PC-014` | Совпадение дублированных швов граней | `IMPLEMENTED` | Runtime seam-map требует полный набор пар (`388/388` при `33 × 33`), сравнивает позиции и нормали одинаковых направлений и выводит `Δpos max`, `Δnormal max`, `PASS/FAIL` |
| `PC-020` | Независимые визуальные поверхности как основа quadtree | `IMPLEMENTED` | Шесть отдельных face mesh; дальнейшее разбиение может выполняться по грани без изменения модели координат |
| `PC-021` | Отдельная collision-поверхность каждой грани | `IMPLEMENTED` | Шесть `ConcavePolygonShape3D` под общим `StaticBody3D` |
| `PC-030` | Диагностика граней и нормалей | `IMPLEMENTED` | HUD, face colors, `F1` для radial normals, орбитальная камера и `Space` для паузы |
| `PC-ACC-001` | Проект собирается без ошибок | `IN_PROGRESS` | Нужна локальная сборка текущего снимка |
| `PC-ACC-002` | Сцена запускается и показывает планету | `IN_PROGRESS` | Нужен screenshot/runtime-подтверждение |
| `PC-ACC-003` | HUD показывает `Грани: 6/6` и `collision: 6/6` | `IN_PROGRESS` | Проверить в `TASK-028` |
| `PC-ACC-004` | Seam-check показывает `PASS` | `IN_PROGRESS` | Проверить `Δpos max` и `Δnormal max` в HUD/Output |
| `PC-ACC-005` | Нет видимых разрывов между гранями | `IN_PROGRESS` | Осмотреть вращающуюся планету в face-color и normals режимах |
| `PC-ACC-006` | Диагностические переключатели работают | `IN_PROGRESS` | `F1` меняет режим, `Space` ставит orbit на паузу и возобновляет его |

Гравитация к центру, ходьба по поверхности, floating origin и quadtree LOD пока не реализованы и будут добавляться следующими изолированными итерациями.
...
**Зафиксировано как `VERIFIED` по прямому подтверждению пользователя:** `TASK-005`, `TASK-009`, `TASK-011`, `TASK-023`, `TASK-024`, `TASK-025`, `TASK-026`; Прототипы A и B.

| Приоритет | ID | Задача | Результат |
|---:|---|---|---|
| 1 | `TASK-028` | Выполнить runtime smoke-test основы cube sphere | Сборка без ошибок; HUD `6/6`, collision `6/6`, `Швы: PASS`; нет видимых разрывов; `F1` и `Space` работают |
| 2 | `TASK-006` | Записать SHA контрольного коммита | Журнал содержит Git-доказательство принятой редакции Прототипов A/B и текущей основы C |
| 3 | `TASK-029` | Добавить радиальную гравитацию и ориентацию игрока относительно центра | Игрок притягивается к сфере, локальный Up направлен от центра, камера и движение не зависят от глобальной оси Y |
| 4 | `TASK-030` | Добавить ходьбу по collision-поверхности cube sphere | Игрок устойчиво перемещается через границы граней без скачка ориентации |

**Реализовано в этой итерации:** закрытие `TASK-026`, завершение Прототипа B и первая геометрическая основа Прототипа C.  
**Текущая задача:** локально выполнить `TASK-028` по разделу 10.
...
2. Нажать `F5`; должна открыться сцена `CubeSpherePrototype`.
3. Убедиться, что Output содержит строку `CubeSphere foundation` без исключений.
4. HUD должен показать `Грани: 6/6`, `collision: 6/6`, сетку `33×33`, `6534` вершины и `12288` треугольников.
5. Строка швов должна показать `Швы: PASS`, `пары: 388/388`; значения `Δpos max` и `Δnormal max` должны быть ниже заданных порогов.
6. Осмотреть вращающуюся планету: все шесть цветов должны последовательно появляться, между гранями не должно быть отверстий, перевёрнутых треугольников или просвечивания внутрь.
7. Нажать `F1`: цвета граней должны смениться визуализацией радиальных нормалей; геометрия и число collision не должны измениться.
8. Нажать `Space`: орбитальное вращение должно остановиться; повторное нажатие должно его возобновить.
9. Проверить Output на отсутствие C# exceptions, `InvalidOperationException`, ошибок mesh/collision и сообщений о несуществующих узлах.
10. При успешном результате передать screenshot HUD и итоговую строку `CubeSphere foundation` для перевода `TASK-027`, `PC-001`–`PC-030` и `PC-ACC-001`–`PC-ACC-006` в `VERIFIED`.
11. Для регрессии Прототипа B открыть `Scenes/Terrain/TerrainChunkPrototype.tscn` и нажать `F6`; краткая проверка `F10` остаётся доступной, но повторный 120-секундный soak не обязателен без изменений terrain-кода.
12. После коммита записать SHA в `TASK-006`.
