...
> **Подготовленный снимок:** `ProjectHorizon-main-floating-origin-build-hotfix.zip`
...
| C. Сферическая планета | `IN_PROGRESS` | Cube sphere, радиальная система и ходьба через независимые collision-грани подтверждены runtime; реализован floating origin, ожидающий локальной приёмки |
| D. Корабль | `NOT_STARTED` | Не начинался |
| E. Сохранение | `NOT_STARTED` | Не начинался |

**Вывод:** пользователь предоставил итоговый screenshot `TASK-030 seam (T): PASS crossings=4, gap=0,00 с, Δup=0,59°` и подтвердил ручную проверку движения, камеры, управления и reset. Поэтому `TASK-030`, `TASK-031`, `PC-050`–`PC-053` и `PC-ACC-020`–`PC-ACC-024` переведены в `VERIFIED`. Текущая итерация реализует `TASK-032`: ячейки `4096 м`, порог `2048 м`, синхронный перенос локальной сцены и контроль непрерывности логических координат.
...
| `PC-060` | Локальная позиция хранится как cell `long` + `Vector3` | `IMPLEMENTED` | `FloatingOriginController`: `CellX/CellY/CellZ`, `LocalPosition`, logical double coordinates |
| `PC-061` | Ячейка 4096 м и порог переноса 2048 м | `IMPLEMENTED` | Экспортируемые `CellSize=4096`, `ShiftThreshold=2048`; положительные и отрицательные cell delta |
| `PC-062` | Все участники локальной сцены переносятся синхронно | `IMPLEMENTED` | Planet, PlanetaryPlayer и CameraRig получают одинаковую translation |
| `PC-063` | Логическая позиция и сохранённые точки не меняются при rebase | `IMPLEMENTED` | Continuity check в double; `NotifyWorldTranslated` корректирует spawn/test transforms |
| `PC-064` | Автоматический floating-origin acceptance test | `IMPLEMENTED` | `Y`: 4 shifts, 6 cell transitions, контроль local/logical/relative/contact и восстановление baseline |
...
| `PC-ACC-030` | Редакция floating origin собирается и запускается | `IN_PROGRESS` | Выполнить локальную C#-сборку без exceptions |
| `PC-ACC-031` | Acceptance test выполняет 4 origin shifts и 6 cell transitions | `IN_PROGRESS` | HUD: `TASK-032 origin (Y): PASS shifts=4, cells=6` |
| `PC-ACC-032` | Локальная координата остаётся внутри ±2048 м | `IN_PROGRESS` | Итоговый `localMax <= 2048 м`; Output показывает rebase по обеим знакам |
| `PC-ACC-033` | Logical coordinate непрерывна | `IN_PROGRESS` | `logicalErr <= 0,01 м`, continuity error в Output не растёт |
| `PC-ACC-034` | Относительные трансформы и контакт сохраняются | `IN_PROGRESS` | `relativeErr <= 0,001 м`, `gap <= 0,12 с`, без визуального рывка |
| `PC-ACC-035` | После теста восстановлены baseline и предыдущие функции | `IN_PROGRESS` | Ячейка/local возвращены; работают `R`, WASD, Space, F1, F2 и T |

Ходьба через границы принята. Floating origin реализован и ожидает runtime-приёмки; quadtree LOD будет следующей изолированной кодовой итерацией.
...
**Зафиксировано как `VERIFIED` по прямому подтверждению пользователя:** `TASK-005`, `TASK-009`, `TASK-011`, `TASK-023`–`TASK-031`; Прототипы A и B; геометрия, радиальная система и collision-швы Прототипа C.

| Приоритет | ID | Задача | Результат |
|---:|---|---|---|
| 1 | `TASK-034` | Выполнить runtime-приёмку floating origin | `TASK-032 origin (Y): PASS`, 4 shifts, 6 cell transitions, local ≤ 2048 м, logical/relative continuity и сохранённый contact |
| 2 | `TASK-033` | Начать quadtree LOD граней и согласование соседей | Независимые patches, LOD-переходы и диагностируемые швы без отверстий |
| 3 | `TASK-006` | Записать SHA контрольного коммита | Журнал содержит Git-доказательство принятой редакции |

**Реализовано в этой итерации:** приёмка `TASK-030/TASK-031` и кодовая часть `TASK-032`.
**Текущая задача:** выполнить локальный floating-origin smoke-test по разделу 10.
...
2. Нажать `F5`; сцена должна запуститься в player-camera mode без C# exceptions.
3. Дождаться `Радиальная система: PASS`, `ground=да` и `probe=да`.
4. Убедиться, что строка floating origin показывает `cell=(0,0,0)`, локальную позицию около поверхности планеты и `shifts=0` при первом запуске.
5. Нажать `Y` один раз. Не использовать WASD, мышь, `Space`, `F2`, `R` или `T` до завершения.
6. Тест должен последовательно выполнить четыре uniform translations и четыре origin-shift события; суммарно ожидается шесть переходов ячеек.
7. Ожидаемый HUD: `TASK-032 origin (Y): PASS shifts=4, cells=6`.
8. Допустимые пределы: `localMax <= 2048 м`, `logicalErr <= 0,01 м`, `relativeErr <= 0,001 м`, `gap <= 0,12 с`; `Δup` не более `3°`.
9. После PASS cell/local должны вернуться к baseline; игрок, планета и камера должны остаться визуально неподвижными относительно друг друга.
10. В Output должны быть четыре строки `Floating origin shift` и итоговая строка `TASK-032 floating-origin acceptance PASS`; exceptions, physics errors и отсутствующие nodes недопустимы.
11. Проверить после теста WASD, `Space`, `R`, `F1`, `F2` и `T`. Для регрессии seam-test должен снова завершаться `TASK-030 ... PASS` либо сохранять ранее подтверждённое корректное поведение.
12. Повторное `Y` во время RUNNING должно остановить тест и восстановить baseline; `F2`, `R` и `T` должны делать то же безопасно.
13. Передать screenshot итогового HUD и скопировать итоговую строку PASS из Output. После этого `TASK-032`, `TASK-034`, `PC-060`–`PC-064` и `PC-ACC-030`–`PC-ACC-035` можно перевести в `VERIFIED`.
